...
export function apply(ctx) {
  ...
  const state = { enabled: true, mode: 'only', providers: [], quantization: 'off' }
  ...
  async function loadState() {
    ...
    try {
      ...
      if (parsed.quantization === 'off' || QUANT_LEVELS.includes(parsed.quantization)) {
        state.quantization = parsed.quantization
      }
    } catch (error) {
  ...
  async function persistState() {
    ...
    try {
      ...
      await fsSvc.writeText(
        target,
        JSON.stringify({ enabled: state.enabled, mode: state.mode, providers: state.providers, quantization: state.quantization }, null, 2),
        undefined,
        undefined,
        policy,
      )
        ...
        if (req.method === 'POST') {
          ...
          if (input && typeof input === 'object') {
            ...
            if (input.quantization === 'off' || QUANT_LEVELS.includes(input.quantization)) {
              state.quantization = input.quantization
            }
          }
          await persistState()
          json(res, 200, {
  ...
  function buildBody(options) {
    ...
    if (state.enabled && (state.providers.length > 0 || state.quantization !== 'off')) {
      const providerParams = {}
      if (state.providers.length > 0) {
        if (state.mode === 'order') {
          providerParams.order = state.providers.slice()
          providerParams.allow_fallbacks = true
        } else {
          providerParams.only = state.providers.slice()
          providerParams.allow_fallbacks = false
        }
      }
      if (state.quantization !== 'off') {
        providerParams.quantizations = [state.quantization]
      }
  ...
  hostCtx.on('llm/stream', (options, next) => {
    if (options.provider === OPENROUTER_ROUTE && state.enabled && (state.providers.length > 0 || state.quantization !== 'off')) {
