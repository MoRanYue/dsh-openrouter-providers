 ...
 *
 * 职责：
 * - 通过 settings 服务注册命名空间 openrouter-providers（schema 见下文），
 *   配置持久化到 DSH 设置文档（settings.yaml），与其它插件一致。命名空间
 *   同时作为 settings.plugin.item 卡片的 key，使「插件 → 插件配置」标签页
 *   能渲染本插件卡片（Host 已服务的 settings 命名空间集合包含该 key）。
 * - 通过 webServer 注册 HTTP API：GET/POST /api/openrouter-providers/state
 *   （供 Client 半读写配置，读写经 settings 服务的 scope.update / scope.get）；
 ...
 */
// dsh-settings >= 0.1.2-alpha.2 dropped the settingsNamespace() helper;
// register() now takes the plain hyphenated namespace string directly.
...
// Settings-service schema. The namespace doubles as the card key registered
// under settings.plugin.item, so the settings UI shows our card when this
// namespace is served by the settings service.
const SettingsSchema = z.object({
  enabled: z.boolean().default(true),
  mode: z.union(['only', 'order']).default('only'),
  providers: z.array(z.string()).default([]),
  quantization: z.union(['off', ...QUANT_LEVELS]).default('off'),
})
...
export function apply(ctx) {
  ctx.inject(['fs', 'sandboxPolicy', 'settings', 'llm', 'webServer', 'timer'], async (hostCtx) => {
  // Live config comes from the settings service (namespace SETTINGS_NS).
  // Serving this namespace is what makes the settings.plugin.item card
  // (key = SETTINGS_NS) render under 插件 → 插件配置.
  const settingsSvc = hostCtx.get('settings')
  if (settingsSvc === undefined || typeof settingsSvc.register !== 'function') {
    console.log('[openrouter-providers] settings service unavailable, plugin inactive')
    return
  }
  const scope = settingsSvc.register(SETTINGS_NS, SettingsSchema)
  const readState = () => scope.get()
  const documentPath = typeof settingsSvc.documentPath === 'string' ? settingsSvc.documentPath : undefined

  // One-time migration from the legacy fs-backed state file (v1.0.4 and
  // earlier). Runs only when the settings user layer for this namespace is
  // still empty (defaults), seeding it with the old file's values once.
  ...
  if (fsSvc !== undefined && stateFile !== undefined) {
    try {
      let userLayer
      try {
        const desc = settingsSvc.describe({ redactSecrets: true }).find(d => d.ns === SETTINGS_NS)
        userLayer = desc === undefined ? undefined : desc.user
      } catch (_descUnavailable) {
        userLayer = undefined
      }
      if (userLayer === undefined) {
        ...
        if (parsed !== null && typeof parsed === 'object') {
          ...
          if (parsed.mode === 'order' || parsed.mode === 'only') patch.mode = parsed.mode
          if (Array.isArray(parsed.providers)) {
            patch.providers = parsed.providers
              .filter(p => typeof p === 'string')
              .map(p => p.trim())
              .filter(p => p.length > 0)
          }
          if (parsed.quantization === 'off' || QUANT_LEVELS.includes(parsed.quantization)) {
            patch.quantization = parsed.quantization
          }
          if (Object.keys(patch).length > 0) await scope.update(patch)
          console.log('[openrouter-providers] migrated legacy state file into settings document')
        }
      }
    } catch (error) {
      console.log('[openrouter-providers] no legacy state to migrate (' + (error && error.message ? error.message : String(error)) + ')')
    }
  }
      ...
      handler: async (req, res) => {
        if (req.method === 'GET') {
          ...
          json(res, 200, {
            enabled: s.enabled,
            mode: s.mode,
            providers: s.providers.slice(),
            quantization: s.quantization,
            ...(documentPath !== undefined ? { stateFile: documentPath } : {}),
          })
          return
        }
        if (req.method === 'POST') {
          ...
          if (input && typeof input === 'object') {
            ...
            if (input.mode === 'order' || input.mode === 'only') patch.mode = input.mode
            ...
            if (input.quantization === 'off' || QUANT_LEVELS.includes(input.quantization)) {
              patch.quantization = input.quantization
            }
          }
