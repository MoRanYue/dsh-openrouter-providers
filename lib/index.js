...
/**
 * HTTP API 与请求注入装配。
 * @param ctx - Host context（可选服务均通过 ctx.get 获取）。
 */
export async function apply(ctx) {
  ...
  if (webServer !== undefined) {
    ...
    ctx.effect(() => dispose, 'dsh-openrouter-providers: state HTTP route')
  }
  ...
  const adapter = {
    ...
    async * stream(options) {
      ...
      const timeoutDispose = ctx.timeout(() => handle.terminate(), TOTAL_MS)
      try {
        let buffer = ''
  ...
  ctx.effect(() => llm.registerAdapter([PLUGIN_ROUTE], adapter))
