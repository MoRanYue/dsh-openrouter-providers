  ...
  return {
    ...
    apply(ctx) {
      ...
      ctx.inject(['slots'], () => {
        ...
        //
        // locale 是可选服务（旧宿主没有），所以不能用 ctx.inject(['locale'])
        // 等待——那会让卡片在没有 locale 的宿主上永不出现。改在此处读取：
        // settings.plugin.item 由 ui-settings-plugins 声明，而它自己注入
        // locale，因此该声明存在即可判定宿主提供本地化能力。
