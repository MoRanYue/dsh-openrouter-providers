 ...
 *
 * 注册插件配置卡片（「插件」→「插件配置」，settings.plugin.item 槽），
...
window.__ModuleLoader__.load({ id: 'dsh-openrouter-providers', factory: (require) => {
  ...
  const css = [
    ...
    '.orpv-page button{background:var(--dsw-alias-button-primary-fill);color:var(--dsw-alias-label-primary-foreground);border:none;border-radius:6px;padding:8px 18px;font-size:13px;cursor:pointer}',
  ...
  function OpenRouterProvidersPage() {
    ...
    React.useEffect(() => {
      ...
      api('GET').then((value) => {
        ...
        setQuantization(typeof v.quantization === 'string' ? v.quantization : 'off')
    ...
    const save = () => {
      ...
      setSaving(true)
      setStatus('')
      api('POST', { enabled, mode, providers, quantization }).then((value) => {
        const v = value && typeof value === 'object' ? value : {}
    ...
    return React.createElement('div', { className: 'orpv-page' }, [
      ...
      React.createElement('p', { key: 'h', className: 'orpv-hint' },
        '当模型请求路由到 OpenRouter（provider 路由 "openrouter"）时，把下面的列表作为 provider 路由参数注入请求体。设置持久化到 DSH 设置文档（settings.yaml），重启后自动恢复。'),
